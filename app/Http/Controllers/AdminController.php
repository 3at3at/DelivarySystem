...
class AdminController extends Controller
{
    ...
    public function reports()
    {
        ...
        $totalRevenue = User::with(['deliveries' => function ($query) {
            $query->where('status', 'Delivered');
        }])->get()->sum(function ($user) {
            return $user->deliveries->sum('price');
        });
        ...
        $driverStats = Driver::withCount([
            ...
            $driver->total_earnings = Delivery::where('driver_id', $driver->id)
        ...
        ->withSum(['deliveries as deliveries_sum_price' => function($query) {
            $query->where('status', 'Delivered');
        }], 'price')
        ->orderByDesc('deliveries_count')
        ->take(5)
        ...
        $allDeliveries = Delivery::with(['client', 'driver'])->orderByDesc('created_at')->get();

        $activeDrivers = Driver::whereHas('deliveries', function ($query) {
            $query->whereIn('status', ['In Progress', 'Accepted'])
